 ...
 *     "version"             =>"1.0",
 *     "use"                 => new \app\admin\controller\ActivityOrderController();
 * )
 */
...
class ActivityOrderController extends AdminBaseController
{
    ...
    protected function base_edit()
    {
        ...
        $ActivityOrderInit = new \init\ActivityOrderInit();//activity_order     (ps:InitController)
        $this->assign('type_list', $ActivityOrderInit->type);
    }
    ...
    public function index()
    {
        ...
        /** 导出数据 **/
        if ($params["is_export"]) $ActivityOrderInit->export_excel($where, $params);


        /** 查询数据 **/
        $result = $ActivityOrderInit->get_list_paginate($where, $params);
    ...
    public function add_post()
    {
        ...
        /** 检测参数信息 **/
        $validateResult = $this->validate($params, 'ActivityOrder');
        if ($validateResult !== true) $this->error($validateResult);


        /** 插入数据 **/
        $result = $ActivityOrderInit->admin_edit_post($params);
    ...
    public function edit_post()
    {
        ...
        /** 提交数据 **/
        $result = $ActivityOrderInit->admin_edit_post($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function edit_post_two()
    {
        ...
        /** 提交数据 **/
        $result = $ActivityOrderInit->edit_post_two($params, $where);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function audit_post()
    {
        ...
        $params["InterfaceType"] = "admin";//接口类型
        $params["DataFormat"]    = "find";//数据格式,find详情,list列表
        $item                    = $ActivityOrderInit->get_find($where);
    ...
    public function delete()
    {
        ...
        /** 删除数据 **/
        $result = $ActivityOrderInit->delete_post($id);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function batch_post()
    {
        ...
        //提交编辑
        $result = $ActivityOrderInit->batch_post($id, $params);
        if (empty($result)) $this->error("失败请重试");
    ...
    public function list_order_post()
    {
        ...
        //提交更新
        $result = $ActivityOrderInit->list_order_post($params);
        if (empty($result)) $this->error("失败请重试");
