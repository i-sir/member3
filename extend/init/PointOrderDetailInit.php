...
/**
    ...
    *     "version"         =>"1.0",
    *     "use"             => new \init\PointOrderDetailInit();
    * )
    */
...
class PointOrderDetailInit extends Base
{
...
public $status =[0=>'未提交',1=>'审核中',2=>'已通过',3=>'已驳回'];//退款状态 
    ...
    protected $DataFormat    = "find";//数据格式,find详情,list列表
        ...
        {
            $PointOrderDetailInit  = new \init\PointOrderDetailInit();//订单详情   (ps:InitController)
            $PointOrderDetailModel = new \initmodel\PointOrderDetailModel(); //订单详情  (ps:InitModel)
        }
        ...
        public function common_item($item = [], $params = [])
        {
            ...
            /** 处理文字描述 **/
            $item['is_vip_name']=$this->is_vip[$item['is_vip']];//是否是会员 
$item['status_name']=$this->status[$item['status']];//退款状态 
 ...
 $item['user_info']=$user_info;
            ...
            if ($this->InterfaceType == 'api') {
                /** api处理文件 **/
                if($item['image']) $item['image']=cmf_get_asset_url($item['image']);//展示图
    ...
    */
    public function get_join_list($where=[], $params = [])
    {
        ...
        */
        public function edit_post($params,$where=[])
        {
          ...
          if(!empty($where)){
                ...
                $result                = $PointOrderDetailModel->where($where)->strict(false)->update($params);
                //if ($result) $result = $item["id"];
            } elseif (!empty($params["id"])) {
                //如传入id,根据id编辑数据
                $params["update_time"] = time();
                $result                = $PointOrderDetailModel->where("id","=",$params["id"])->strict(false)->update($params);
                //if($result) $result = $item["id"];
           } else {
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $PointOrderDetailModel->strict(false)->insert($params,true);
            }
        ...
        */
        public function edit_post_two($params,$where=[])
        {
            ...
            if(!empty($where)){
                ...
                //无更新条件则添加数据
                $params["create_time"] = time();
                $result                = $PointOrderDetailModel->strict(false)->insert($params);
            }
        ...
        */
        ...
        if ($type == 1) $result = $PointOrderDetailModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $PointOrderDetailModel->destroy($id,true);//真实删除
        ...
        */
        public function batch_post($id, $params=[])
        {
            ...
            $params["update_time"] = time();
            $result = $PointOrderDetailModel->where($where)->strict(false)->update($params);//修改状态
        ...
        */
        public function list_order_post($list_order,$params=[])
        {
            ...
            foreach ($list_order as $k => $v) {
                $where   = [];
                $where[] = ["id", "=", $k];
                $result  = $PointOrderDetailModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
            }
        ...
        */
        public function export_excel($where = [], $params = [])
        {
            ...
            $result       = $PointOrderDetailInit->get_list($where, $params);
            ...
            $Excel->excelExports($result, $headArrValue, ["fileName" => "订单详情"]);
        }
