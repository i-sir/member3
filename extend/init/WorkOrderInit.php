...
class WorkOrderInit extends Base
{

    public $status = [1 => '待支付', 2 => '已报名', 3 => '已到岗', 4 => '已驳回', 5 => '已弃岗', 10 => '已取消'];//状态
    ...
    public function common_item($item = [], $params = [])
    {
        ...
        /** 处理文字描述 **/
        $item['type_name']   = $WorkInit->type[$item['type']];//类型
        $item['p_type_name'] = $WorkInit->p_type[$item['p_type']];//支付方式
    ...
    public function edit_post($params, $where = [])
    {
        ...
        //处理时间格式
        if ($params['max_refund_time'] && is_string($params['max_refund_time'])) $params['max_refund_time'] = strtotime($params['max_refund_time']);//24小时后不退款
        if ($params['pass_time'] && is_string($params['pass_time'])) $params['pass_time'] = strtotime($params['pass_time']);//通过时间
        if ($params['onboard_time'] && is_string($params['onboard_time'])) $params['onboard_time'] = strtotime($params['onboard_time']);//到岗时间


        if (!empty($where)) {
            //传入where条件,根据条件更新数据
            $params["update_time"] = time();
            $result                = $WorkOrderModel->where($where)->strict(false)->update($params);
            //if ($result) $result = $item["id"];
        } elseif (!empty($params["id"])) {
            //如传入id,根据id编辑数据
            $params["update_time"] = time();
            $result                = $WorkOrderModel->where("id", "=", $params["id"])->strict(false)->update($params);
            //if($result) $result = $item["id"];
        } else {
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $WorkOrderModel->strict(false)->insert($params, true);
        }
    ...
    public function edit_post_two($params, $where = [])
    {
        ...
        if (!empty($where)) {
            ...
            //无更新条件则添加数据
            $params["create_time"] = time();
            $result                = $WorkOrderModel->strict(false)->insert($params);
        }
    ...
    public function delete_post($id, $type = 1, $params = [])
    {
        ...
        if ($type == 1) $result = $WorkOrderModel->destroy($id);//软删除 数据表字段必须有delete_time
        if ($type == 2) $result = $WorkOrderModel->destroy($id, true);//真实删除
    ...
    public function batch_post($id, $params = [])
    {
        ...
        $params["update_time"] = time();
        $result                = $WorkOrderModel->where($where)->strict(false)->update($params);//修改状态
    ...
    public function list_order_post($list_order, $params = [])
    {
        ...
        foreach ($list_order as $k => $v) {
            $where   = [];
            $where[] = ["id", "=", $k];
            $result  = $WorkOrderModel->where($where)->strict(false)->update(["list_order" => $v, "update_time" => time()]);//排序
        }
    ...
    public function export_excel($where = [], $params = [])
    {
        ...
        $result = $WorkOrderInit->get_list($where, $params);
        ...
        $Excel->excelExports($result, $headArrValue, ["fileName" => "报名岗位"]);
    }
